...
class LifeScienceBot:
    ...
    def verify_and_summarize(self, wiki_data, pubmed_data):
        """用DeepSeek整合并强调证据等级（修复版无Markdown）"""
...
📖 为什么？
（解释机制，100字内，用大白话，不要用星号）

🔬 证据来源（必须明确标注）：
• ✅ 强证据（来自：{pubmed_data['journal'] if pubmed_data else 'Cochrane系统评价'}）：一句话结论
• 📚 参考知识（来自：Wikipedia/WHO指南）：背景补充
• ⚠️ 注意事项：什么人群不适用？

❌ 常见谣言澄清（重要）：
（针对这个主题，市面上流传的错误说法，用❌标记辟谣，不要用横线）

【严禁】：
• 禁止出现"中医认为"、"专家表示"等模糊来源
• 禁止推荐保健品/具体品牌
• 禁止绝对化表述（"一定"、"肯定"），改用"研究显示"
• 禁止任何Markdown符号
"""
